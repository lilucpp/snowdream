...
int main() {  // Client code
  ...
  DogNature dogNature;
  //	dogNature.carryOutNature (catFemale);  // Will not compile of course
  //since the parameter must be of type Dog*.
  ConversionAdapter* adaptedCat = new ConversionAdapter(
      catFemale);  // catFemale has adapted to become a Dog!

  dogNature.carryOutNature(dogFemale);
  dogNature.carryOutNature(
      adaptedCat);  // So now catFemale, in the form of adaptedCat, participates
                    // in the dogNature! Note that catFemale is carrying out her
                    // own type of nature in dogNature though.
